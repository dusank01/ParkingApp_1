...
export const authUser = async (): Promise<boolean> => {
  try {
    ...
    Alert.alert(
      "Greška",
      "Došlo je do greške prilikom autentifikacije. Pokušajte ponovo.",
    );
    return false;
  }
};
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (!isBeaconDetected) {
        Alert.alert(
          "Greška",
          "Nije detektovan Bluetooth uređaj. Pokušajte ponovo.",
        );
        resolve(false);
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        Alert.alert("Uspešno", "Bluetooth uređaj je detektovan.");
        resolve(true);
      }
    }, 1000);
  });
